...
export function ChannelFilterList({
  ...
  const resolvedEmptyLabel = emptyLabel ?? t('noChannels');
  const resolvedSearchPlaceholder = searchPlaceholder ?? t('filterChannels');
  ...
  return (
    <div className="border border-border bg-input">
      ...
      <div className="max-h-72 divide-y divide-border overflow-y-auto">
        {visibleItems.length === 0 ? (
          <p className="px-3 py-6 text-center text-sm text-muted-foreground">
            {items.length === 0 ? resolvedEmptyLabel : t('noMatchingChannels')}
          </p>
        ) : (
            ...
            return (
              <div key={item.value} className="flex min-h-12 items-center gap-3 px-3 py-2">
                ...
                <div
                  ...
                  aria-label={t('channelPolicyLabel', { channel: item.label })}
                >
                  <button
                    type="button"
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-border px-3 py-2 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
        <span>{t('allowedCount', { count: allowed.length })}</span>
        <span>{t('ignoredCount', { count: ignored.length })}</span>
        <span className="ml-auto normal-case tracking-normal">
          {allowed.length === 0
            ? t('allChannelsAllowed')
            : t('onlyAllowedChannels')}
        </span>
      </div>
    </div>
  );
